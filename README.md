...
Voor deze analyse is RNA-sequence data gebruikt van vier RA-patiënten en vier gezonde controles. De ruwe reads (FASTQ-bestanden) bevinden zicht in [Raw_Data](Data/raw/) Deze zijn uitgelijnd op het humane referentiegenoom (NCBI RefSeq: GCF_000001405.40) met behulp van het `Rsubread`-pakket in  [1_preprocessing_indexing](Scripts/1_preprocessing_indexing.R ), waarna `.BAM`-bestanden zijn gegenereerd. Deze zijn gersorteerd en geïndexeerd in [02_sort_index_counts](Scripts/02_sort_index_counts.R) en vervolgens met `featureCounts()`  een gen-telling uitgevoerd op basis van een GTF-bestand, resulterend in een count-matrix.
